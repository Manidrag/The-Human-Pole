// poll-frontend/src/components/PollCreator.js
...
const PollCreator = ({ onPollCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!creatorName.trim() || !creatorEmail.trim() || !question.trim() || validOptions.length < 2) {
      alert("Please enter your name, email, a question, and at least two options.");
      return;
    }
    try {
      const response = await fetch('http://localhost:3000/api/polls', {
        ...
        body: JSON.stringify({ creatorName, creatorEmail, question, options: validOptions })
      });
      if (response.ok) {
